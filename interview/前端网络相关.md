...
3. http缓存？
   1. 分为强制缓存和协商缓存
      - 强制缓存（Pragma，Cache-Control，Expires），优先级从高到低分别是 **Pragma -> Cache-Control -> Expires**
      - 协商缓存 （Last-Modified，If-Modified-Since和If-Unmodified-Since）和（ETag，If-None-Match和If-Match）
   2. 协商缓存
      - 服务端缓存策略
      - 服务器判断客户端资源，是否和服务端资源一样
      - 一致则返回304，否则返回200和新的资源
   3. ![image-20201219094605580](/Users/renhaolei/Library/Application Support/typora-user-images/image-20201219094605580.png)
      4. 刷新操作对缓存的影响？
         - 正常操作：地址栏输入URL，跳转链接，前进后退等；（强制缓存✅，协商缓存✅）
         - 手动刷新：F5，点击刷新按钮，左右菜单刷新；（强制缓存❌，协商缓存✅）
         - 强制刷新：ctrl+F5（强制缓存❌，协商缓存❌）